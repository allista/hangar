...
[*][b]Ground hangars[/b] (simple and habitable) for colonies [*][b]Rover Lander[/b] hangar that has all needed modules and fuel to autonomously land on a planet or moon, bringing some rovers along the way [*]there's also the [b]Spaceport[/b] that combines a huge hangar with a cockpit; as such, the Spaceport has only a single stack node at its bottom [/list] 
